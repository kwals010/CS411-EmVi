...
/*SLIDESHOW TILE*/
slideshowTiles = [];
nextSlideshow=function(id,imgs,texts,effect,s,dir){
		var $id = $("#"+id);
		...
		if(s!=0){
			timers[id] = setTimeout(function(){nextSlideshow(id,imgs,texts,effect,s)},s);
		}
	}
$.plugin($siteLoad,{	
	slideShowInit:function(){
		for(var i in slideshowTiles){
			...
			nextSlideshow(i,slideshowTiles[i][0],slideshowTiles[i][1],slideshowTiles[i][2],slideshowTiles[i][3]);
		}
		$(".tileSlideshow").on("mouseover","#sl_arrowLeft, #sl_arrowRight",function(){
			...
			nextSlideshow(id,slideshowTiles[id][0],slideshowTiles[i][1],slideshowTiles[id][2],slideshowTiles[id][3],-1);
			return false;
			...
			id = $id.attr("id")
			$id.data("n",($id.data("n")+1));
			nextSlideshow(id,slideshowTiles[id][0],slideshowTiles[id][1],slideshowTiles[id][2],slideshowTiles[i][3],1);
			return false;
		});
	}
});